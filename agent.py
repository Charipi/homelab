...
def off():
    ...
    return execute_remote_python(script_path=f'{config["RASPI_PATH"]}solid.py', args=["0", "0", "0"])
...
def set_color(red: int, green: int, blue: int) -> str:
    ...
    return execute_remote_python(script_path=f'{config["RASPI_PATH"]}solid.py', args=[str(red), str(green), str(blue)])
...
def set_color_loop(primary_red: int, primary_green: int, primary_blue: int, secondary_red: int, secondary_green: int, secondary_blue: int) -> str:
    ...
    return execute_remote_python(script_path=f'{config["RASPI_PATH"]}loop.py', args=[str(primary_red), str(primary_green), str(primary_blue), str(secondary_red), str(secondary_green), str(secondary_blue), "20", "1"])
